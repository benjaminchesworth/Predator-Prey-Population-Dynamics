...
1. Running `git clone https://github.com/benjaminchesworth/Predator-Prey-Population-Dynamics.git` in your terminal, or by
...
- The designed flow of the system is to first decide on the parameters for an experiment run, run it, do post-experiment analysis, and then save the experiment and analysis data. This data can either be passed or extracted from file to be displayed in different forms.
  ...
  - `EnAData`, defined in `EnAData.py` is constructed by passing in `ExperimentData` and `AnalysisData`.
    ...
    - `FiledExperimentData` defined in `ExperimentData.py`, and `FiledAnalysisData` defined in `AnalysisData.py`, provide info and useful methods for filed data.
    - The `FiledEnAData` class, defined in `EnAData.py`, contains both `FiledExperimentData` and `FiledAnalysisData` and provides methods to handle the combined filed data.
